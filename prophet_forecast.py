...
def run_prophet_forecast(
    ...
    forecast["yhat"] = forecast["yhat"].clip(lower=0, upper=y_cap)
    forecast["yhat_lower"] = forecast["yhat_lower"].clip(lower=0, upper=y_cap)
    forecast["yhat_upper"] = forecast["yhat_upper"].clip(lower=0, upper=y_cap)
    ...
    validation_start = test_actual.index[0]
    model_forecast = forecast_series[forecast_series.index >= validation_start]

    test_pred = model_forecast.loc[test_actual.index.intersection(model_forecast.index)]
    ...
    st.subheader("\U0001F4CA Forecast Accuracy Metrics")
    ...
    st.subheader("\U0001F50E Display Settings")
    max_periods = len(df_resampled)
    display_periods = st.slider(
        f"\U0001F4C6 Show last N {freq.lower()} periods of historical data + forecast",
        min_value=7,
    ...
    st.subheader("\U0001F4C8 A1 Sales Forecast - Prophet")
    ...
    ax.plot(model_forecast.index, model_forecast.values, color="green", linestyle="--", marker="x", label="Model Forecast")
    for x, y in zip(model_forecast.index, model_forecast.values):
    ...
    st.download_button(
        label="\U0001F4E5 Download Prophet Forecast CSV",
        data=model_forecast.to_csv().encode(),
